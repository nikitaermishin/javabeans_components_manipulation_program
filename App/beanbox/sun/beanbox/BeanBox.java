...
public class BeanBox extends JPanel implements Serializable, Runnable,
        ...
        bcss.addService(
                ...
                new BeanContextServiceProvider() {
                    ...
                    public Object getService(BeanContextServices bcs, Object requestor, Class serviceClass, Object serviceSelector) {
                        if (serviceSelector != null && ((serviceSelector instanceof String && (String) serviceSelector != ibn) || true))
                            return null;
                        return ib;
                    }
    ...
    public synchronized void updateMenuBar(JMenuBar bar) {
        BeanBoxFrame frame = getFrame();

        if (bar.getMenuCount() == 0) {
            // Create the basic menus.  These are bean/beanbox independent.

            JMenu m = new JMenu("File"); // menu 0

            addMenuItem(frame, m, new JMenuItem("Save..."));
            addMenuItem(frame, m, new JMenuItem("SerializeComponent..."));
            addMenuItem(frame, m, new JMenuItem("MakeApplet..."));
            addMenuItem(frame, m, new JMenuItem("Load..."));
            addMenuItem(frame, m, new JMenuItem("LoadJar..."));
            addMenuItem(frame, m, new JMenuItem("Print"));
            addMenuItem(frame, m, new JMenuItem("Clear"));
            addMenuItem(frame, m, new JMenuItem("Exit"));
            bar.add(m);

            m = new JMenu("Edit"); // menu 1
            addMenuItem(frame, m, new JMenuItem("Cut"));
            addMenuItem(frame, m, new JMenuItem("Copy"));
            pasteMenuItem = new JMenuItem("Paste");
            pasteMenuItem.setEnabled(false);
            addMenuItem(frame, m, pasteMenuItem);
            addMenuItem(frame, m, new JMenuItem("Report..."));
            bar.add(m);

            m = new JMenu("View"); // menu 2
            bar.add(m);
			...
			m = new JMenu("Help"); // menu 5 (was menu 4! (or was it the help menu...))
            addMenuItem(frame, m, new JMenuItem("About..."));
            addMenuItem(frame, m, new JMenuItem("Documentation..."));
            bar.add(m);
            // TODO uncomment bar.setHelpMenu(m);
        }

        JMenu editMenu = bar.getMenu(EDIT_MENUID);
        ...
        try {
            ...
            try {
                ...
                if (customizerClass != null) {
                    addMenuItem(frame, editMenu, new JMenuItem("Customize..."));
                }
            } catch (Exception ex) {
            ...
            if (esd.length > 0) {
                ...
                for (int i = 0; i < esd.length; i++) {
                    ...
                    JMenu methodMenu = new JMenu(dname);
            ...
            if (bindable) {
                addMenuItem(frame, editMenu, new JMenuItem("Bind property..."));
            }
    ...
    private void addMenuItem(BeanBoxFrame frame, JMenu m, JMenuItem mi) {
    ...
    void doMenuItem(ActionEvent evt) {
        ...
        JMenu parent = (JMenu) popup.getInvoker();
        ...
        if (parent.getText().equals("File")) {
        ...
        } else if (parent.getText().equals("Edit")) {
            if (label.equals("Cut")) {
                cut();
        ...
        } else if (parent.getText().equals("View")) {
        ...
        } else if (parent.getText().equals("Help")) {
            ...
            if (label.equals("InfoBus Support...")) {
                ...
                mt.setVisible(true);
                mi.setText("Hide Method Tracing");
            } else if (label.equals("Hide Method Tracing")) {
                mt.setVisible(false);
                mi.setText("Show Method Tracing");
            }
        } else {
    ...
    void resetViewMenu() {
        BeanBoxFrame frame = getFrame();
        JMenu viewMenu = getFrame().getJMenuBar().getMenu(VIEW_MENUID);
        viewMenu.removeAll();
        if (Beans.isDesignTime()) {
            addMenuItem(frame, viewMenu, new JMenuItem("Disable Design Mode"));
        } else {
            addMenuItem(frame, viewMenu, new JMenuItem("Enable Design Mode"));
        }
        if (BeanBoxFrame.getHideInvisibleBeans()) {
            addMenuItem(frame, viewMenu, new JMenuItem("Show Invisible Beans"));
        } else {
            addMenuItem(frame, viewMenu, new JMenuItem("Hide Invisible Beans"));
        }
    }
    ...
    transient JMenuItem pasteMenuItem;
